...
def sigma_from_target(target, cfg=None):
    cfg = cfg or {}
    kind = cfg.get("kind", "resolution")
    if kind != "resolution":
        raise ValueError(f"Unknown sigma kind: {kind}")
    ...
    t = torch.clamp(target, min=1e-6)
    sigma2 = (stochastic * torch.sqrt(t)) ** 2 + (constant * t) ** 2 + noise ** 2
    return torch.clamp(torch.sqrt(sigma2), min=sigma_min)


def compute_loss(pred, target, weight_cfg=None, denom_min=1.0, loss_name="smooth_l1_relative", sigma_cfg=None):
